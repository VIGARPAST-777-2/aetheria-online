...
function preload() {
  ...
  g.fillRect(24, 40, 16, 20);
  g.fillStyle(0xc4a35a);
  g.fillRect(10, 28, 10, 10);
...
function create() {
  ...
  this.add.image(18 * 32, 12 * 32, 'house').setDepth(3);
  this.add.image(32 * 32, 11 * 32, 'house').setDepth(3);
  this.add.image(25 * 32, 26 * 32, 'house').setDepth(3);
  ...
  npcs.forEach(n => this.physics.add.collider(player, n.sprite));
...
function update() {
  ...
  if (cursors.left.isDown || wasd.left.isDown || m.left) vx = -speed;
  else if (cursors.right.isDown || wasd.right.isDown || m.right) vx = speed;
  if (cursors.up.isDown || wasd.up.isDown || m.up) vy = -speed;
  else if (cursors.down.isDown || wasd.down.isDown || m.down) vy = speed;
